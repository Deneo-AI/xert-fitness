...
test('native home opens with the photographic XERT website hero language', async () => {
  ...
  assert.match(home, /lineLimit\(dynamicTypeSize\.isAccessibilitySize \? 4 : 2\)/);
  assert.match(home, /lineLimit\(dynamicTypeSize\.isAccessibilitySize \? 6 : 3\)/);
  assert.match(home, /lineLimit\(dynamicTypeSize\.isAccessibilitySize \? 3 : 2\)/);
  assert.match(home, /heroPhotoURLs\.count <= 5[\s\S]*heroPhotoURLs\.indices/);
  assert.match(home, /\.frame\(width: 44, height: 44\)/);
  assert.match(home, /Previous training photo/);
  assert.match(home, /Next training photo/);
  assert.ok(home.includes('Text("\\(visiblePhotoIndex + 1) of \\(heroPhotoURLs.count)")'));
  assert.match(home, /private func selectPhoto\(_ requestedIndex: Int\)/);
  assert.match(home, /requestedIndex % heroPhotoURLs\.count \+ heroPhotoURLs\.count/);
  assert.match(home, /private var visiblePhotoIndex: Int/);
  assert.match(home, /heroPhotoURLs\.indices\.contains\(photoIndex\) \? photoIndex : 0/);
  assert.match(home, /XertHaptics\.play\(\.selection\)/);
...
test('native hero stays functional and leaves member operations directly below it', async () => {
  ...
  assert.match(home, /NativeHomeHero[\s\S]*NativeValueStrip[\s\S]*announcementsSection[\s\S]*NativeTrainingIdentity[\s\S]*todayTrainingSection/);
